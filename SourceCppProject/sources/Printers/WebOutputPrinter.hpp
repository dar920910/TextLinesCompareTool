#ifndef WEB_OUTPUT_PRINTER_HPP_
#define WEB_OUTPUT_PRINTER_HPP_
...
class WebOutputPrinter : public AbstractOutputDevice
{
private:
    std::ofstream m_WebOutputFileStream;

    const std::string generateOutputFileName() const;

    const std::string generateWebPageBegin();
    const std::string getDocumentCharset(const std::string& charset);
    const std::string getDocumentName(const std::string& title);
    const std::string getStyles();
    const std::string generateWebPageEnd();
...
public:
    ...
    void printArtifacts(const LinesResultMapBasedView& result_artifact);
    void printArtifacts(const LinesResultSetBasedView& result_artifact);
};
